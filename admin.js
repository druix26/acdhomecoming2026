...
function render() {
  ...
  tableBody.innerHTML = rows.map((row) => `<tr><td><strong>${escapeHtml(row.reference)}</strong><small>Batch ${escapeHtml(row.batch_year)}</small></td><td><strong>${escapeHtml(row.full_name)}</strong><small>${escapeHtml(row.email_address)}</small><small>${escapeHtml(row.mobile_number)}</small></td><td title="${escapeHtml(attendeeNames(row))}"><strong>${row.total_attendees} attendee${row.total_attendees===1?'':'s'}</strong><small>${escapeHtml((row.guest_names||[]).join(', ')||'No guests')}</small></td><td><strong>${money(row.amount_paid)}</strong><small>${escapeHtml(row.payment_method)}</small><small>${escapeHtml(row.transaction_reference)}</small></td><td><span class="status ${row.status==='Confirmed'?'confirmed':''}">${escapeHtml(row.status)}</span></td><td>${date(row.submitted_at)}</td><td>${row.receipt_url?`<a class="receipt" href="${escapeHtml(row.receipt_url)}" target="_blank" rel="noopener">View receipt ↗</a>`:'Unavailable'}</td><td><input class="verify-registration" type="checkbox" aria-label="Verify ${escapeHtml(row.full_name)}" data-id="${escapeHtml(row.id)}" ${row.status==='Confirmed'?'checked disabled':''}></td></tr>`).join('');
