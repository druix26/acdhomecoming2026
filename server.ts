...
async function sendConfirmedRegistrationEmail(registration: Record<string, unknown>) {
  ...
  const guestHtml = guests.length
    ? guests.map((guest) => `<tr><td style="padding:9px 0;border-bottom:1px solid #d9dde5;font-size:18px;color:#17213b">&#10003;&nbsp;&nbsp;${escapeHtml(guest)}</td></tr>`).join("")
    : `<tr><td style="padding:9px 0;font-size:18px;color:#667085">No guests listed</td></tr>`;
  const welcomeText = guests.length
    ? "We can't wait to welcome you and your guests for an unforgettable evening of stories, laughter, and reconnection."
    : "We can't wait to welcome you for an unforgettable evening of stories, laughter, and reconnection.";
  const resend = new Resend(apiKey);
